exports.up = function (knex) {
    return knex.schema.createTable("task_submissions", (table) => {
        ...
        table.bigInteger("submitted_by").notNullable().references("id").inTable("users");
        table.string("note").nullable();
        table.enu("review_status", ["pending", "approved", "revision_requested", "rejected"]).notNullable().defaultTo("pending");
